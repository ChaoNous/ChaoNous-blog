...
`+`.is-theme-transitioning .expressive-code:not(.hide-during-transition) {
        ...
        opacity: 1 !important;
      }`,this.tempStyleSheet.textContent=t)}}initCodeBlockOptimization(){this.codeBlockObserver=new IntersectionObserver(t=>{t.forEach(e=>{e.isIntersecting?(this.visibleBlocks.add(e.target),this.pendingThemeUpdate&&this.applyThemeToBlock(e.target,this.pendingThemeUpdate)):this.visibleBlocks.delete(e.target)})},{rootMargin:"50px 0px",threshold:.01}),this.observeCodeBlocks(),this.setupThemeListener(),window.swup&&window.swup.hooks.on("page:view",()=>{setTimeout(()=>this.observeCodeBlocks(),100)})}observeCodeBlocks(){this.visibleBlocks.clear(),requestAnimationFrame(()=>{document.querySelectorAll(".expressive-code").forEach(e=>{this.codeBlockObserver.observe(e),this.hideCodeBlocksDuringTransition?e.classList.add("hide-during-transition"):e.classList.remove("hide-during-transition")})})}setupThemeListener(){new MutationObserver(e=>{for(const i of e)if(i.type==="attributes"&&i.attributeName==="data-theme"){const s=document.documentElement.getAttribute("data-theme");this.handleThemeChange(s);break}}).observe(document.documentElement,{attributes:!0,attributeFilter:["data-theme"]})}handleThemeChange(t){this.pendingThemeUpdate=t;const e=Array.from(this.visibleBlocks);e.length!==0&&this.batchUpdateBlocks(e,t)}batchUpdateBlocks(t,e){let s=0;const o=()=>{const n=t.slice(s,s+3);requestAnimationFrame(()=>{n.forEach(r=>{this.applyThemeToBlock(r,e)}),s+=3,s<t.length&&setTimeout(o,0)})};o()}applyThemeToBlock(t,e){t.dataset.themeUpdated=e}interceptThemeSwitch(){new MutationObserver(e=>{for(const i of e)if(i.type==="attributes"&&i.attributeName==="class"&&i.target===document.documentElement){const s=document.documentElement.classList,o=s.contains("is-theme-transitioning"),n=s.contains("use-view-transition");o&&!this.isOptimizing?this.optimizeThemeSwitch(n):!o&&this.isOptimizing&&this.restoreAfterThemeSwitch(n)}}).observe(document.documentElement,{attributes:!0,attributeFilter:["class"]})}optimizeThemeSwitch(t=!1){this.isOptimizing=!0,this.useViewTransition=t,!t&&(this.disableHeavyAnimations(),this.hideOffscreenHeavyElements(),this.forceCompositing())}disableHeavyAnimations(){this.tempStyleSheet||(this.tempStyleSheet=document.createElement("style"),this.tempStyleSheet.id="theme-optimizer-temp",document.head.appendChild(this.tempStyleSheet)),this.tempStyleSheet.textContent=`
      /* 临时禁用重型元素的过渡和动画 */
    ...
    `}hideOffscreenHeavyElements(){const t=window.innerHeight,e=window.scrollY;this.hiddenElements=[];const i=[];this.heavySelectors.forEach(s=>{document.querySelectorAll(s).forEach(n=>{i.push({element:n,rect:n.getBoundingClientRect()})})}),i.forEach(({element:s,rect:o})=>{const n=o.top+e;if(n+o.height<e-200||n>e+t+200){const a=s.style.contentVisibility;s.style.contentVisibility="hidden",this.hiddenElements.push({element:s,originalVisibility:a})}})}forceCompositing(){const t=document.querySelectorAll(`
      .expressive-code,
      .post-card,
      .widget,
      #navbar
    `);this.compositedElements=[],t.forEach(e=>{const i=e.style.transform;e.style.transform="translateZ(0)",e.style.willChange="transform",this.compositedElements.push({element:e,original:i})})}restoreAfterThemeSwitch(t=!1){if(this.isOptimizing=!1,t){this.useViewTransition=!1;return}requestAnimationFrame(()=>{requestAnimationFrame(()=>{this.tempStyleSheet&&this.tempStyleSheet.parentNode&&(this.tempStyleSheet.remove(),this.tempStyleSheet=null),this.hiddenElements&&(this.hiddenElements.forEach(({element:e,originalVisibility:i})=>{e.style.contentVisibility=i||""}),this.hiddenElements=null),this.compositedElements&&(this.compositedElements.forEach(({element:e,original:i})=>{e.style.transform=i||"",e.style.willChange=""}),this.compositedElements=null)})})}destroy(){this.codeBlockObserver&&this.codeBlockObserver.disconnect(),this.visibleBlocks.clear()}}const l=new h;window.themeOptimizer=l;
